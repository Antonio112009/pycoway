...
class CowayAuthClient(CowayHttpClient):
        ...
        async with self._session.post(
            ...
            if page_title == 'Coway - Password change message':
                if self.skip_password_change:
                    form_url = soup.find(
                        'form', id='kc-password-change-form'
                    ).get('action')
            ...
            if (
                error_message
                and error_message.text == 'Your ID or password is incorrect.'
            ):
                raise AuthError(
    ...
    async def _get_login_cookies(self) -> tuple[str, Any]:
        ...
        if status != 200:
            ...
            raise CowayError(
                f'Coway API error while fetching login page. '
                f'Status: {status}, Reason: {error}'
            )
    ...
    async def _get_auth_code(self, login_url: str, cookies: Any) -> str:
        ...
        response, password_skip_init = await self._post_auth(
            login_url, cookies, headers, data
        )
    ...
    async def _get_token(self, auth_code: str) -> tuple[str, str]:
        ...
        if 'error' in response:
            LOGGER.debug(
                f'Error obtaining access/refresh token for {self.username}. '
                f'Response: {response}'
            )
            ...
            raise CowayError(
                f'Failed fetching Coway access token: '
    ...
    async def _check_token(self) -> None:
        ...
        if not self.check_token:
            LOGGER.debug(
                f'Token check is set to False. Skipping for {self.username}'
            )
            return
        ...
        if any(
            v is None
            for v in [self.access_token, self.refresh_token, self.token_expiration]
        ):
            LOGGER.debug(
    ...
    async def _refresh_token(self) -> None:
        ...
        if 'error' in response:
            LOGGER.debug(
                f'Error refreshing tokens for {self.username}. Response: {response}'
            )
            await self.login()
            return
        ...
        if self.access_token is None or self.refresh_token is None:
            raise CowayError(
                f'Failed to refresh tokens for {self.username}. Response: {response}'
            )

        self.token_expiration = datetime.now() + timedelta(seconds=3600)
        LOGGER.debug(
            f'Tokens refreshed for {self.username}. '
            f'New expiration: {self.token_expiration}'
        )
    ...
    async def _get_country_code(self) -> str:
        ...
        response = await self._get_endpoint(
            endpoint=endpoint, headers=headers, params=None
        )
        ...
        if 'error' in response:
            raise CowayError(
                f'Failed to get country code associated with account. '
                f'{response["error"]}'
            )

        raise CowayError(
            f'Unexpected response getting country code for {self.username}. '
            f'Response: {response}'
        )
    ...
    async def _get_places(self) -> list[dict[str, Any]]:
        ...
        if 'error' in response:
            raise CowayError(
                f'Failed to get places associated with account. {response["error"]}'
            )
        ...
        raise NoPlaces(
            f'No places found associated with {self.username}. Response: {response}'
        )
