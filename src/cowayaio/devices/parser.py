...
def parse_purifier_html(html: str, nick_name: str) -> dict[str, Any] | None:
    ...
    try:
        ...
        LOGGER.debug(
            f'Parsed purifier JSON info: {json.dumps(purifier_json, indent=4)}'
        )
    except Exception as exc:
        raise CowayError(
            f'Failed to parse purifier HTML page for info: {exc}'
        ) from exc

    if 'children' not in purifier_json:
        LOGGER.debug(
            f'No children key found for purifier {nick_name}. '
            f'Setting purifier info to None.'
        )
        return None
...
def extract_parsed_info(purifier_info: dict[str, Any]) -> dict[str, Any]:
    ...
    if 'baseInfoForModelCodeData' in purifier_info:
        parsed['device_info'] = purifier_info['baseInfoForModelCodeData'].get(
            'deviceInfo', {}
        )
