...
class CowayDataClient(CowayMaintenanceClient):
    ...
    async def async_get_purifiers(self) -> list[dict[str, Any]]:
        ...
        for place in self.places:
            ...
            try:
                ...
                LOGGER.debug(
                    'Access and refresh tokens are invalid. Fetching new tokens.'
                )
                await self.login()
                headers = await self._create_endpoint_header()
                response = await self._get_endpoint(url, headers, params)
            ...
            if devices:
                purifiers.extend(
                    d for d in devices if d['categoryName'] == CATEGORY_NAME
                )
            else:
                LOGGER.debug(
    ...
    async def async_get_purifiers_data(self) -> PurifierData:
        ...
        LOGGER.debug(
            f'Purifiers found for {self.username}: '
            f'{json.dumps(purifiers, indent=4)}'
        )
        if not purifiers:
            raise NoPurifiers(
                f'No purifiers found for any IoCare+ places '
                f'associated with {self.username}.'
            )
    ...
    async def async_fetch_filter_status(
        ...
        url = (
            f'{Endpoint.SECONDARY_BASE}{Endpoint.PLACES}/'
            f'{place_id}/devices/{serial}/supplies'
        )
        headers = {
            'region': 'NUS',
    ...
    async def async_fetch_timer(
        self, serial: str, name: str
    ) -> dict[str, Any]:
        ...
        if 'error' in response:
            raise CowayError(
                f'Failed to get timer for purifier {name}: {response["error"]}'
            )
