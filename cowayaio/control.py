...
class CowayControlClient(CowayDataClient):
    ...
    def _validate_control_response(
        self, response: dict[str, Any] | str, command_name: str
    ) -> None:
        ...
        if isinstance(response, dict):
            ...
            raise CowayError(
                f'Failed to execute {command_name} command. Response: {response}'
            )
    ...
    async def _send_control(
        ...
        LOGGER.debug(
            f'{device_attr.name} - {command_name} command sent. '
            f'Response: {response}'
        )
        self._validate_control_response(response, command_name)

    async def async_set_power(
        self, device_attr: DeviceAttributes, is_on: bool
    ) -> None:
        """Provide is_on as True for On and False for Off."""
        await self._send_control(
            device_attr, '0001', '1' if is_on else '0', 'power'
        )
    ...
    async def async_set_fan_speed(
        self, device_attr: DeviceAttributes, speed: str
    ) -> None:
    ...
    async def async_set_light(
        self, device_attr: DeviceAttributes, light_on: bool
    ) -> None:
        """Provide light_on as True for On and False for Off.
        NOT used for 250s purifiers.
        """
        await self._send_control(
            device_attr, '0007', '2' if light_on else '0', 'light'
        )
    ...
    async def async_set_timer(
        self, device_attr: DeviceAttributes, time: str
    ) -> None:
    ...
    async def async_set_smart_mode_sensitivity(
        ...
        await self._send_control(
            device_attr, '000A', sensitivity, 'smart mode sensitivity'
        )

    async def async_set_button_lock(
        self, device_attr: DeviceAttributes, value: str
    ) -> None:
    ...
    async def async_change_prefilter_setting(
        ...
        LOGGER.debug(
            f'{device_attr.name} - Prefilter command sent. Response: {response}'
        )
